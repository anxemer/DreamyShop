...
public class DAO {
    ...
    public List<Game> getFreeGame() {
        List<Game> list = new ArrayList<>();
        String query = "SELECT Game.*, CategoryGame.nameCategory FROM Game\n" +
                    "JOIN CategoryGame ON CategoryGame.id = Game.cateId\n" +
                    "where price = 0";
        try {
            conn = new DBcontext().getConnection();
    ...
    public void signup(String fullname, String user, String pass, String email) {
...
public void addGame(String name,String price,String image,String category,String image1,String image2,String image3,String image4,
                        String video,String description,String os,String processor,String memory,String graphics,String storage){
        String query = "INSERT Game  \n" +
                        "VALUES (?,?,?,?)\n" +
                         "INSERT Infomation "
                + "VALUES (?,? ,?,? ,?,? )"
                + "INSERT [dbo].[Configuration]  VALUES ( ?, ?, ?, ?,?)" ;
        try {
            conn = new DBcontext().getConnection();
            ps = conn.prepareStatement(query);
            ps.setString(1, name);
